...
const days: Day[] = [
  ...
  { date: 'Sep 2', day: 'Wed · together', theme: 'Museum SAN, Wonju', description: 'The full-day architecture pilgrimage: Tadao Ando, landscape, James Turrell, Meditation Hall and Antony Gormley.', stops: [{ name: 'Museum SAN', korean: '강원특별자치도 원주시 지정면 오크밸리2길 260 뮤지엄산', note: 'Leave Seoul early; keep evening open' }], must: 'Buy the Signature Pass if available.' },
  ...
  { date: 'Sep 5', day: 'Sat · together', theme: 'Slow Jongno + fireworks', description: 'Keep the day light, rest in the afternoon, then go early to Nodeul Island.', stops: [{ name: 'Nodeul Island Orange Play Zone', korean: '서울 용산구 노들섬', note: 'Fireworks; wristband exchange closes 17:00', booking: 'https://www.hanwhafireworks.com/' }], must: 'Book Orange Play Zone tickets if still available.' },
...
export default function Home() {
  ...
  return <main>
    ...
    <nav className="filters" aria-label="Trip site navigation"><button className={tab === 'itinerary' ? 'selected' : ''} onClick={() => setTab('itinerary')}>Itinerary</button><button className={tab === 'explore' ? 'selected' : ''} onClick={() => setTab('explore')}>Explore nearby</button><a className="tab-link" href="#travel-tips">Travel Tips</a></nav>
    {tab === 'itinerary' ? <>
